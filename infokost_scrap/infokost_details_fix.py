...
def scrap(site: str, driver_path: str) -> int:
    ...
    try:    
        ...
        # for p in range(1):  ## for testing purpose
            ...
            print(info_listing) 
            file = open(r'20231113_surabaya.csv', 'w+', newline ='')
            with file:
            ...
            for l in  link_properti_list_ind:
                ...
                try:
                    ...
                    df_dict1 = pd.DataFrame([dict1])
                    df2 = pd.concat([df2, df_dict1], ignore_index=True)
                    df2.to_csv("infokost_details_surabaya.csv", index=False, encoding='utf-8', sep='|')
                    print(f"Total records saved: {len(df2)}")




        print(site, '- Success')
        # file = open(r'surabaya.csv', 'w+', newline ='')
        # with file:
        #     write = csv.writer(file)
        #     write.writerows(info_listing)
    except Exception:
        ...
        driver.quit()
        if not df2.empty:
            df2.to_csv("infokost_details_surabaya_fix.csv", index=False, encoding='UTF-8', sep='|')
            print(f"Total records saved: {len(df2)}")
        else:
            print("No data was collected from df2 final")
